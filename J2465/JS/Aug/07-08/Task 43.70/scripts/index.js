...
$(document).ready(() => {
  ...
  $("#user-table").on("click", ".voucher-history", function () {
    redirectToVoucherHistory($(this).closest("tr"));
  });
});
...
function redirectToVoucherHistory($row) {
  const queryParams = new URLSearchParams({
    username: $row.find("td:eq(1)").html(),
    firstname: $row.find("td:eq(2)").html(),
    lastname: $row.find("td:eq(3)").html(),
  ...
  window.location.href = `voucherHistory.html?${queryParams.toString()}`;
}
  ...
  columnDefs: [
    ...
    },
    {
      ...
      defaultContent: `
        ...
        <i class="fas fa-gift voucher-history" style='color: blue; cursor: pointer''></i>        
        `,
    },
  ],
});
